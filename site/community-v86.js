...
let activeChannel=(location.hash||'#global').slice(1);
let userScrollTop=0,userNearBottom=true,preserveScroll=false;
let forumOrderSeed=0;
...
function initComposer(){const input=$('#messageInput'),counter=$('#messageCounter'),form=$('#messageForm'),emojiButton=$('#emojiButton'),emojiTray=$('#emojiTray');if(!input)return;
  ...
  const restore=()=>{try{const value=localStorage.getItem(draftKey())||'';if(!input.value&&value)input.value=value}catch{}grow();count()};
  input.addEventListener('input',()=>{grow();count();save()});input.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.isComposing&&matchMedia('(pointer:fine)').matches){e.preventDefault();if(input.value.trim()||!$('#fileStrip')?.hidden)form?.requestSubmit()}});
  form?.addEventListener('submit',()=>{setTimeout(()=>{try{localStorage.removeItem(draftKey())}catch{}grow();count()},80)});
  const emojis=['😀','😂','😍','🔥','✨','👍','❤️','🎉','🙏','😎','🤝','✅','👀','💡','🚀','🎮','⛏️','🌿'];
  ...
  window.addEventListener('hashchange',()=>{activeChannel=(location.hash||'#global').slice(1);setTimeout(restore,100)});restore();
}
...
function initForumTools(){const list=$('#forumList'),search=$('#forumSearchInput'),sort=$('#forumSort'),empty=$('#forumEmpty');if(!list||!search||!sort)return;
  const stamp=()=>{$$('.c84-post',list).forEach((p,i)=>{if(!p.dataset.c86Order)p.dataset.c86Order=String(++forumOrderSeed+i/1000)})};
  const apply=()=>{stamp();const q=search.value.trim().toLocaleLowerCase('id-ID'),posts=$$('.c84-post',list);posts.forEach(p=>p.hidden=!!q&&!p.textContent.toLocaleLowerCase('id-ID').includes(q));if(sort.value==='popular')posts.sort((a,b)=>postScore(b)-postScore(a)||Number(a.dataset.c86Order)-Number(b.dataset.c86Order)).forEach(p=>list.appendChild(p));else posts.sort((a,b)=>Number(a.dataset.c86Order)-Number(b.dataset.c86Order)).forEach(p=>list.appendChild(p));const visible=posts.filter(p=>!p.hidden).length;if(empty)empty.hidden=!q||visible>0};
  ...
  new MutationObserver(()=>{if($$('.c84-post',list).length){stamp();if(search.value||sort.value!=='latest')apply()}}).observe(list,{childList:true});
}
