...
function isViewClick(e){return !!e.target.closest?.('[data-view]')}
...
function initSearch(){const toggle=$('#communitySearchToggle'),panel=$('#communitySearchPanel'),input=$('#communitySearchInput'),meta=$('#communitySearchMeta'),clear=$('#communitySearchClear'),empty=$('#communitySearchEmpty');if(!toggle||!panel||!input)return;
  ...
  document.addEventListener('click',e=>{if(isViewClick(e))setTimeout(close,0)});
...
function initComposer(){const input=$('#messageInput'),counter=$('#messageCounter'),form=$('#messageForm'),emojiButton=$('#emojiButton'),emojiTray=$('#emojiTray');if(!input)return;let lastValue=input.value,currentDraftKey=draftKey();
  ...
  const save=(key=currentDraftKey)=>{try{const value=input.value;if(value)localStorage.setItem(key,value);else localStorage.removeItem(key)}catch{}};
  const sync=()=>{lastValue=input.value;grow();count();save()};
  const restore=()=>{let value='';try{value=localStorage.getItem(currentDraftKey)||''}catch{}input.value=value;lastValue=value;grow();count()};
  const switchDraft=()=>{const next=draftKey();if(next===currentDraftKey)return;save(currentDraftKey);currentDraftKey=next;restore()};
  ...
  document.addEventListener('click',e=>{if(isViewClick(e))setTimeout(switchDraft,0)});window.addEventListener('hashchange',switchDraft);
  ...
  document.addEventListener('click',e=>{if(emojiTray&&!emojiTray.hidden&&!emojiTray.contains(e.target)&&e.target!==emojiButton){emojiTray.hidden=true;emojiButton?.setAttribute('aria-expanded','false')}});restore();
}
...
function initScrollPreservation(){const scroller=$('#messageScroller'),list=$('#messageList'),jump=$('#jumpToLatest');if(!scroller||!list||!jump)return;
  ...
  const reset=()=>{jump.hidden=true;userNearBottom=true;userScrollTop=0};
  ...
  document.addEventListener('click',e=>{if(isViewClick(e))setTimeout(reset,0)});window.addEventListener('hashchange',reset);
}
...
function initForumTools(){const list=$('#forumList'),search=$('#forumSearchInput'),sort=$('#forumSort'),empty=$('#forumEmpty');if(!list||!search||!sort)return;
  ...
  const reset=()=>{search.value='';sort.value='latest';forumOrderSeed=0;if(empty)empty.hidden=true};
  ...
  search.addEventListener('input',apply);sort.addEventListener('change',apply);document.addEventListener('click',e=>{if(isViewClick(e))setTimeout(reset,0)});window.addEventListener('hashchange',reset);
