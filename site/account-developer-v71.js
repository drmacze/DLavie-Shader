(() => {
  ...
  function addProfileAccess(){
    ...
    if(identity&&!$('#developerConsoleChip')) identity.insertAdjacentHTML('beforeend',`<a id="developerConsoleChip" class="developer-console-chip" href="developer.html?v=71">${icon}<span>Console</span></a>`);
    const profile=$('.profile-header');
    const signout=$('#signOutButton');
    if(profile&&signout&&!$('#developerConsoleButton')) signout.insertAdjacentHTML('beforebegin',`<a id="developerConsoleButton" class="developer-console-button" href="developer.html?v=71">${icon}<span>Developer Console</span></a>`);
    ...
    if(quick&&!$('#developerQuickLink')){
      ...
      kicker?.insertAdjacentHTML('afterend',`<a id="developerQuickLink" class="developer-quick-link" href="developer.html?v=71">${icon}<span>Developer Console</span><b>→</b></a>`);
    }
  }
  ...
  function addMobileAccess(){
    ...
    a.href='developer.html?v=71';
    a.dataset.dlvDeveloperConsole='true';
    a.className='dlv-developer-console-row';
    a.innerHTML=`<span class="dlv-dev-menu-icon">${icon}</span><span class="dlv-dev-menu-copy"><strong>Developer Console</strong><small>${label(access.role)}</small></span><span class="dlv-menu-arrow">›</span>`;
